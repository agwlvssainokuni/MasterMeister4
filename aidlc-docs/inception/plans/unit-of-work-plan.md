...
A) 機能単位（package-by-feature）: 例 `cherry.mastermeister.registration`, `cherry.mastermeister.accesscontrol`, `cherry.mastermeister.query` のように、ユニット／ドメインごとにパッケージを分ける。1ユニットの変更が1パッケージに閉じやすい

B) レイヤ単位（package-by-layer）: 例 `cherry.mastermeister.controller`, `cherry.mastermeister.service`, `cherry.mastermeister.repository` のように技術層で分ける

X) Other（[Answer]: の後に内容を記述）

[Answer]: A（ベースパッケージは `cherry.mastermeister` とする）
