...
6. クエリ保存・実行（P5、Epic6+7。クエリ手入力での実行・保存クエリの実行という核機能を先に成立させる）
7. クエリビルダー（P5、Epic5。PBT対象を含む。ユニット6の実行・保存機能に連携する形で本実装する）
8. クエリ履歴（P5、Epic8。ユニット6・7で実行・ビルダーが揃った後、記録の蓄積と遷移先の両方が揃った状態で着手する）
9. 監査ログ閲覧（P4〜P5、Epic9）
10. CI/CD（GitHub Actions・タグpushリリース、最終ユニット、NFR-10.x）

**ユニット6〜8の順序根拠**: クエリビルダー（Epic5）は生成SQLの実行・保存への連携（FR-5.6）で実行・保存の両方に依存し、履歴（Epic8）は実行による記録の蓄積（FR-8.1）と実行・保存・ビルダーへの遷移（FR-8.4）の両方に依存する。依存される側（保存・実行）を先に、依存する側（ビルダー→履歴の順）を後に配置することで、連携部分を仮実装（スタブ）にせず本実装できる。

---

## 推定タイムライン

AI-DLCはステージ／ユニット単位で進行を管理し、暦日での工数見積りは行わない。上記の見通しでは10ユニット程度になる想定だが、正式な数と粒度はUnits Generationステージで確定する。単独開発者による段階的実装（MVPファースト）のため、各ユニットは前のユニットの完了後に順次着手する想定。
