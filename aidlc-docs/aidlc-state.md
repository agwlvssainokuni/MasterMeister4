...
- [ ] Code Generation — Part 1（計画）作成完了、承認待ち（unit-07-code-generation-plan.md、全12セクション。DTO設計の簡略化（model/dto分離をせず入れ子DTOを共用）を主要な実装判断として明記）
...
- [ ] UNIT-07 クエリビルダー — IN PROGRESS（Functional Design・NFR Requirements・NFR Design承認済み、Infrastructure DesignはSKIP、Code Generation Part 1計画承認待ち）
- [ ] UNIT-08 クエリ履歴
- [ ] UNIT-09 監査ログ閲覧
- [ ] UNIT-10 CI/CD