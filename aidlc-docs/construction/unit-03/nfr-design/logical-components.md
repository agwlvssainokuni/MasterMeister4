...
- `buildJdbcUrl(host, port, databaseName, schemaName, additionalParams)`（レビュー指摘の反映、component-methods.md訂正）: JDBC URLのスキーム・パラメータ区切り文字は方言ごとに異なるため、URL構築自体を本メソッドに集約する
  - `MySqlDialectStrategy`/`MariaDbDialectStrategy`: `jdbc:mysql://{host}:{port}/{databaseName}?{additionalParams}` 形式（`?`＋`&`区切り）
  - `PostgresDialectStrategy`: `jdbc:postgresql://{host}:{port}/{databaseName}?{additionalParams}` 形式（`?`＋`&`区切り。`schemaName`はURLには含めず、接続後の`SET search_path`（`applySchemaSwitch`）で適用する）
