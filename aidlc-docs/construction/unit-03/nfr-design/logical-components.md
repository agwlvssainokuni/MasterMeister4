...
- タイムアウト発生時、実行中の`Connection`を明示的に`close()`し、バックグラウンドスレッドを強制的に中断させる（nfr-design-patterns.md §1.1、レビュー指摘の反映。プール枠の専有継続を防ぐ）
- **追記（UNIT-04 Code Generationにて）**: `refreshSchema()`に`@CacheEvict(cacheNames = "effectivePermission", allEntries = true)`を追加した。主キー構成の変化がUNIT-04の`canCreate`/`canDelete`判定に影響するため（UNIT-04 nfr-design-patterns.md §2.1、BR-ACCESS-08）
...
- ~~`requiresSchemaSwitch()`: PostgreSQL/H2は`true`（schemaName指定時）、MySQL/MariaDBは`false`~~ 訂正（UNIT-04 Functional Designにて）: `requiresSchemaSwitch()`/`applySchemaSwitch()`はスキーマ取込（イントロスペクション）では使用しなくなった（JDBCのDatabaseMetaData取得はschemaPattern引数で直接絞り込めるため）。UNIT-06（クエリ実行時の対象スキーマ指定、FR-7.5）専用のメソッドとする
- `applySchemaSwitch(connection, schema)`: PostgreSQL/H2は`SET search_path`相当の操作を実行（UNIT-06専用、上記訂正参照）
- ~~`buildJdbcUrl(host, port, databaseName, schemaName, additionalParams)`~~ 訂正（UNIT-04 Functional Designにて）: `buildJdbcUrl(host, port, databaseName, additionalParams)`（schemaName引数を削除。1接続内に複数スキーマが存在しうる前提に変更したため、接続確立時点で単一スキーマを指定する意味がなくなった）。JDBC URLのスキーム・パラメータ区切り文字は方言ごとに異なるため、URL構築自体を本メソッドに集約する
  - `MySqlDialectStrategy`/`MariaDbDialectStrategy`: `jdbc:mysql://{host}:{port}/{databaseName}?{additionalParams}` 形式（`?`＋`&`区切り）
  - `PostgresDialectStrategy`: `jdbc:postgresql://{host}:{port}/{databaseName}?{additionalParams}` 形式（`?`＋`&`区切り）
  - `H2DialectStrategy`: `jdbc:h2:tcp://{host}:{port}/{databaseName};{additionalParams}` 形式（`;`区切り。MySQL系とパラメータ区切り文字が異なる点に注意）
  - `additionalParams`（BR-RDBMS-10）は、生の文字列としてそのまま該当区切り文字でURLへ付加する。管理者が入力する際は、選択中の`dbType`に応じた記法（`&`区切り or `;`区切り）で入力する必要がある
  - `isSystemSchema(schemaName)`（UNIT-04 Functional Designにて追加）: 1接続内の複数スキーマ自動検出時、システムスキーマ（PostgreSQLの`information_schema`/`pg_catalog`/`pg_toast*`、H2の`INFORMATION_SCHEMA`）を除外する
...
public record Rdbms(List<EncryptionKey> encryptionKeys) {
    public Rdbms {
        ...
        // keyIdの重複がないことを検証（レビュー指摘の反映。重複は設定ミスの兆候としてfail-fast対象に含める）
        long distinctKeyIds = encryptionKeys.stream().map(EncryptionKey::keyId).distinct().count();
        if (distinctKeyIds != encryptionKeys.size()) {
