...
監査ログの閲覧（大量閲覧・網羅的な閲覧を含む）は、新たな監査記録対象としない。UNIT-02で確立した監査ログ記録方針は一貫して「データ変更を伴う操作」または「セキュリティ上重要な操作」を対象としており、UNIT-05〜08の閲覧系機能（マスタデータ参照、クエリ実行結果参照、クエリ履歴閲覧）もいずれも監査対象としていない。本ユニットもこの既存方針との一貫性を優先し、`AUDIT_LOG_VIEWED`のような新規イベント種別は追加しない。アクセス自体の追跡はアプリケーションログ（SECURITY-03、既存基盤）に委ねる。
