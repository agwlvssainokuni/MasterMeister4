...
- 一覧のページサイズはUNIT-08と同じ既定値・上限値を踏襲する（Q4=A）
...
| SECURITY-14（アラート・監視） | 該当（判断確定） | 監査ログの閲覧自体（大量閲覧含む）は新たな監査記録対象としない（Q2=A）。既存の監査ログ記録方針（データ変更・セキュリティ上重要な操作が対象）との一貫性を優先し、読み取り専用の閲覧機能を監査対象外とするUNIT-05〜08の既存方針をそのまま踏襲する。アクセス自体はアプリケーションログ（SECURITY-03）で追跡可能 |
