...
UNIT-02/UNIT-03で実際に確立した規約（`frontend/src/pages/`・`frontend/src/api/`のフラット構成）に基づき本ユニットも実装する。UNIT-01で予約済みのナビゲーション項目「アクセス制御」（`design-system/components/navigation.ts`、`key: 'accessControl'`, `path: '/access-control'`）を、グループ管理画面として活性化する。

**設計方針（チャットでの議論により確定）**: 接続の選択は権限設定画面への遷移前（接続一覧画面からの導線）で完了させ、権限設定画面内でのDB接続切替は行わない（UNIT-03の`/connections/{id}/schema`と同じルーティング方針）。権限設定画面の基本構造は「プリンシパル主体（ツリー型）」を採用する: プリンシパル（ユーザ／グループ）を選択すると、リソース階層（接続全体／テーブル／カラム）のツリーが展開され、各ノードで権限を直接編集する。

**ツリーUIについて**: 既存のdesign-systemコンポーネントに汎用的なTreeコンポーネントは存在しない。本ユニットの階層（接続全体／テーブル／カラムの3階層、各ノードに主権限セレクト＋補助権限チェックボックスという固有の編集UIを持つ）は`DataTable`の行選択モデルにはなじまないため、新規の汎用Treeコンポーネントを設計システムに追加するのではなく、`AccessPermissionTreePage`固有の展開／折りたたみ表示として実装する（UNIT-03の`SchemaDetailPage`がマスタ・ディテール2画面をそれぞれ`DataTable`で素朴に実装したのと同様、必要最小限の実装を優先する）。

---

## 1. グループ管理画面（`/access-control`、`AppShell`）
...
└ PageHeader（タイトル: 接続の表示名＋「権限設定」、右上 Button「YAMLエクスポート」・Button「YAMLインポート」）
  ...
  │  └ 接続全体ノード（1行。ラベルは接続のスキーマ名、またはMySQL/MariaDBのようにスキーマ概念がない場合は「接続全体」）
  │     - 主権限Select（未設定 / NONE / READ / UPDATE）
  │     - CREATE Checkbox, DELETE Checkbox
  │     └ ▼展開 → テーブルノード（テーブルごとに1行、Icon（chevron）で展開/折りたたみ）
  │         - 主権限Select（未設定 / NONE / READ / UPDATE）
  │         - CREATE Checkbox, DELETE Checkbox
  │         └ ▼展開 → カラムノード（カラムごとに1行）
...
- CREATE/DELETE Checkboxの切替も同様に即時保存する（テーブルノード・接続全体ノードのみ表示。カラムノードには表示しない）
...
UNIT-02で新設したトップ画面の「アクセス制御」カードを活性化する。`HomePage.tsx`の`IMPLEMENTED_KEYS`に`'accessControl'`を追加し、`/access-control`（グループ管理画面）へのリンクとして活性化する。
