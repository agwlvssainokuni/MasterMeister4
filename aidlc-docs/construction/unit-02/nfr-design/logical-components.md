...
- ~~CORS設定（許可オリジンの明示列挙）~~ 訂正（UNIT-02 Code Generationにて）: Vite devサーバのプロキシ導入によりブラウザから見て常に同一オリジンとなるため、CORS設定は不要と判明し削除
...
- Step1エンドポイントのメールアドレス単位レート制限（SECURITY-11、BR-REG-07）。デフォルト閾値は1時間あたり3回
- `RegistrationRateState`エンティティ（domain-entities.md §5、LoginAttemptStateと同様のパターン）で永続化
...
- ~~`spring-boot-starter-mail`の`JavaMailSender`でSMTP送信~~
  ...
  訂正（UNIT-02 Code Generationにて）: 上記3点は横断的インフラであり`registration`固有の関心事ではないと判明し、`common.mail.MailDeliveryService`（新設）へ切り出した。`EmailNotificationService`は`MailDeliveryService`の呼び出しと、送信失敗時のフェイルオープン処理（3.1参照。こちらは業務処理をロールバックさせないという`registration`固有の方針のため残置）のみを担う
- 送信失敗時のフェイルオープン処理（3.1参照）

### MailDeliveryService（`cherry.mastermeister.common.mail`、訂正により新設）
- Mustacheテンプレートレンダリング（`MailTemplateRenderer`経由、`cherry.mustache.Mustache` API呼び出し）
- 件名抽出（BR-MAIL-03の正規表現処理、`MailTemplateRenderer`側）
- `spring-boot-starter-mail`の`JavaMailSender`でSMTP送信（Fromアドレスは`mm.app.mail.from`）
- 送信失敗時は例外を呼び出し元へ送出するのみ（フェイルオープン等の方針は呼び出し元が決定）
