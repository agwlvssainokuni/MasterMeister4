...
無効化／再有効化、および却下取り消しの操作は、管理者ダッシュボードとは別の「ユーザ管理」画面（承認待ちに限らず全ステータスのユーザを対象とする）から行う（frontend-components.md参照）。無効化／再有効化はAuditEventPublisher経由で記録する（BR-AUDIT-02の`USER_DISABLED`/`USER_ENABLED`）。却下取り消し（`REJECTED`→`APPROVED`）は、初回承認と同じ`USER_APPROVED`イベントとして記録する（承認という行為自体は同一であり、遷移元ステータスの違いはイベント種別を分ける理由にならないと判断）。
