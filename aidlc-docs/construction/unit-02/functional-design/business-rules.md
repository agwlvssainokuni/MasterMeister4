...
### BR-REG-07: 登録開始（Step1）エンドポイントのレート制限（SECURITY-11、NFR Design段階で閾値確定）
同一メールアドレスに対して1時間あたり3回までとする（設定可能: `mm.app.user-registration.rate-limit.max-requests`、デフォルト3。時間窓は`mm.app.user-registration.rate-limit.window`、デフォルト1時間）。`LoginAttemptGuard`（BR-LOGIN-01、5回/15分）より緩めの値とする。正規ユーザが「確認メールが届かない」と繰り返し送信し直すケースも想定した値であり、ロックではなく単純な回数上限として扱う（閾値到達後は時間窓が経過するまで新規のトークン発行・メール送信を行わない。BR-REG-04によりAPIレスポンス自体は成功と同一の内容を返す）。
...
- リフレッシュトークンの再利用検知（`TOKEN_REUSE_DETECTED`、BR-TOKEN-02。SECURITY-14のセキュリティイベントアラート対象）
...
ユーザID・対象リソース・補足情報（`detail`）はイベント種別によって意味・書式が異なる。個別の説明はここでは行わず、domain-entities.md §6.1（イベント種別ごとの記録内容）を単一の参照先とする。
