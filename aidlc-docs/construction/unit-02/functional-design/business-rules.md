...
- `APPROVED` → `DISABLED`（管理者による無効化。管理者が任意のタイミングで実行可能）
- `DISABLED` → `APPROVED`（管理者による無効化解除。再有効化を許可する）
- `REJECTED`から他状態への遷移は行わない。却下されたメールアドレスで再度利用したい場合は、Step 1からの新規登録として扱う（新しいUserレコードを作成する。同一メールアドレスでの再登録を妨げない）
...
- 上記の実際のファイルコピュー・Gradle設定変更は、本ユニットのCode Generationステージで実施する（Functional Designでは方針決定のみ）
