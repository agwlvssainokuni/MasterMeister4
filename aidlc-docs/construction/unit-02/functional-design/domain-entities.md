...
| `userId` | UserId（nullable） | 操作を行った主体のID。意味はイベント種別によって異なる（§5.1参照）。ユーザが特定できない場合（ログイン失敗等）はnull |
| `connectionId` | ConnectionId（nullable） | 対象RDBMS接続ID。本ユニットのイベントでは通常null（UNIT-03以降で利用） |
| `eventType` | AuditEventType | `LOGIN` / `LOGOUT` / `LOGIN_FAILURE` / `REGISTRATION_REQUESTED` / `REGISTRATION_COMPLETED` / `USER_APPROVED` / `USER_REJECTED` / `USER_DISABLED` / `USER_ENABLED`（本ユニットで追加する種別。他ユニットが追加する種別は各ユニットのFunctional Designで定義） |
| `targetResource` | String（nullable） | 操作対象を人間可読な形で識別する情報。意味・書式はイベント種別によって異なる（§5.1参照） |
...
### 5.1 イベント種別ごとの記録内容（レビュー指摘の反映、一元化）
