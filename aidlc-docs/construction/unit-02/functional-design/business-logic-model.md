...
2. **無効化時**（`POST /api/admin/users/{id}/disable`）: `APPROVED`ステータスのユーザのみを対象とし、UserステータスをAPPROVED→DISABLEDに遷移させる。以降、当該ユーザはログイン不可となる。当該ユーザに紐づく有効なリフレッシュトークンを全て失効させる（`revokedReason = ADMIN_DISABLED`。FR-3.6により複数端末・複数トークンファミリで同時ログインしている可能性があるため、特定の1トークンファミリではなく該当`userId`の全件を対象とする。不正利用防止のため、無効化時点でセッションを即座に無効にする）
...
2. リフレッシュ要求時（`POST /api/auth/refresh`）:
   ...
   - **再利用検知時（BR-TOKEN-02）**: トークン窃取・再送の兆候とみなし、同一トークンファミリIDに属する全トークンを一括失効させる（revokeFamily()）。以降、当該ファミリでのリフレッシュは一切拒否される。AuditEventPublisher経由で`TOKEN_REUSE_DETECTED`イベントを発行する（Code Generation時に発見した抜けを反映。BR-AUDIT-02参照）
...
2. テンプレートエンジン（Mustache形式、ユーザー提供の自作実装）を用いて、言語別のHTMLテンプレートファイルから本文（HTML全体）を生成する（BR-MAIL-02）
3. レンダリング結果から件名（`<title>`要素）を抽出する（BR-MAIL-03）
4. 件名・本文（HTML）をSMTP経由で送信する（開発環境ではMailPitコンテナ、本番は環境変数によるSMTP設定。NFR-3.1〜3.2、既存決定を踏襲。HTML形式のみ、プレーンテキストのフォールバックは用意しない）
