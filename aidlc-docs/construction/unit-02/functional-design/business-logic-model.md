...
2. UserRegistrationServiceは、同一メールアドレスが既に「承認済み」または「承認待ち」で存在するかを確認する
...
5. 使用済みの登録トークンを無効化する
6. AuditEventPublisher経由で該当イベントを発行する（登録申請自体は1.1で記録済みのため、ここでは重複記録しない。もしくは「登録完了」を別イベント種別として追加するかはbusiness-rules.mdで定義）
...
4. **却下時**（`POST /api/admin/users/{id}/reject`）: UserステータスをPENDING→REJECTEDに遷移させる。却下後、同一メールアドレスでの再登録は新規登録（Step1からやり直し）として扱う（BR-REG-01）
...
## 4. ログイン・ログアウト（FR-3.1〜FR-3.2, FR-3.5〜FR-3.6）
...
5. 検証成功時: LoginAttemptGuard.reset()で失敗カウンタをリセットする。アクセストークン（JWT）とリフレッシュトークンのペアを発行する（§5参照）。AuditEventPublisher経由で「ログイン」イベントを発行する
...
## 5. リフレッシュトークンのローテーション・再利用検知（FR-3.3〜FR-3.4）
...
4. 本ユニットで発行対象とするイベント種別: ログイン、ログアウト、ログイン失敗、アカウント登録申請、管理者によるアカウント承認／却下（§6.1「認証イベント」「管理操作」の該当部分）。他の管理操作・データアクセスイベントは後続ユニット（UNIT-03〜UNIT-08）が同じ基盤を用いて発行する

---

## 8. メール通知の生成（登録確認・承認結果、FR-1.2, FR-1.6）
