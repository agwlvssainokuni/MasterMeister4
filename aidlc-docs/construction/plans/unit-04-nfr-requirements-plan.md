...
[Answer]: C（チャットでの検証により変更: `@CacheEvict`は「1キー削除」または「全件クリア」の2択しか宣言的に表現できず、接続ID単位の部分一致削除はアノテーションのみでは不可能と判明。Q2でSpring Cache抽象化を選んだこととの整合を優先し、無効化は`@CacheEvict(allEntries = true)`によるキャッシュ全体クリアとする（接続単位ではなく全接続分を一括で無効化。権限変更・グループ変更の頻度は低いため許容））
