...
import { SavedQueryConnectionListPage } from './SavedQueryConnectionListPage'
  ...
  return render(
    <ThemeProvider>
      <MemoryRouter initialEntries={['/saved-queries']}>
        <AuthProvider>
          <Routes>
            <Route path="/saved-queries" element={<SavedQueryConnectionListPage />} />
...
describe('SavedQueryConnectionListPage', () => {
  afterEach(() => {
    vi.resetAllMocks()
  })
  ...
  it('行をクリックすると保存クエリ一覧画面へ遷移する', async () => {
    vi.mocked(queryApi.listQueryConnections).mockResolvedValueOnce([
      { connectionId: 1, displayName: '本番DB' },
    ])
