...
// frontend-components.md Flow A-1。保存クエリ管理の起点となる接続選択画面。
export function SavedQueryConnectionListPage() {
  ...
  const columns: readonly TableColumn<AccessibleConnection>[] = [
    { key: 'displayName', header: t('savedQuery.connectionName'), render: (c) => c.displayName },
  ]

  return (
    <AuthenticatedLayout activeNavKey="savedQueries">
      <PageHeader title={t('savedQuery.title')} />
      ...
      {loading ? (
        ...
        <DataTable
          ...
          onRowClick={(c) => navigate(`/saved-queries/${c.connectionId}`)}
          emptyState={<EmptyState message={t('savedQuery.connectionListEmpty')} />}
        />
      )}
    </AuthenticatedLayout>
  )
