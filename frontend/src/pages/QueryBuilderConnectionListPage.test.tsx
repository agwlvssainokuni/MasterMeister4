...
import { QueryBuilderConnectionListPage } from './QueryBuilderConnectionListPage'
...
function renderPage() {
  return render(
    <ThemeProvider>
      <MemoryRouter initialEntries={['/query-builder']}>
        <AuthProvider>
          <Routes>
            <Route path="/query-builder" element={<QueryBuilderConnectionListPage />} />
...
describe('QueryBuilderConnectionListPage', () => {
  afterEach(() => {
    vi.resetAllMocks()
  })

  it('アクセス可能な接続一覧を表示する', async () => {
    vi.mocked(queryApi.listQueryConnections).mockResolvedValueOnce([{ connectionId: 1, displayName: '本番DB' }])
    renderPage()
  ...
  it('行をクリックするとクエリビルダー画面へ遷移する', async () => {
    vi.mocked(queryApi.listQueryConnections).mockResolvedValueOnce([{ connectionId: 1, displayName: '本番DB' }])
